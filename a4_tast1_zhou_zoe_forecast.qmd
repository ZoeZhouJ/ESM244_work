...
```{r}
#| fig-cap: "Figure 3: Seasonplots of Monthly Average Counts for Salmon Passage by Species"
...
For **Coho and Jack Coho salmon**, the highest correlation coefficients occur at lags of **12 months**, indicating strong seasonality. This suggests that **exponential smoothing** could be a suitable forecasting method. Exponential smoothing calculates weighted averages of past observations, with the weights decreasing exponentially as the observations become older. We use the `ETS()` function from the `fable` package to generate predictions. Given the change in variance over time, we specify **multiplicative seasonality**, and since the trend appears linear, we define it as **additive**.

For Steelhead salmon, I do not expect exponential smoothing (ETS) to be the most effective forecasting method due to its broader and more variable seasonal pattern and significant interannual variability. However, we will still apply exponential smoothing to evaluate the impact of model selection on forecast accuracy

```{r}
# Return fish_long to tsibble format
...
fish_fit <- fish_long %>% 
  group_by(species) %>% 
  model(ets = HoltWinters(counts))
        #arima = ARIMA(counts),
        #snaive = SNAIVE(counts))

fish_forecast <- fish_fit %>% 
...
coho_fit <- fish_long %>% 
  ...
  group_by(species) %>% 
  model(
    ets = ETS(counts ~ season(method="M") + trend(method="A"), restrict = FALSE)
  )

# Forecast 5 years beyond 2010
...
#coho_forecast %>% 
#  autoplot(filter(fish_long, species == "coho"))

```
```{r}


# Ensure the data is in tsibble format
fish_long <- fish_long %>% 
  as_tsibble(key = species, index = date)

# Split the data by species
species_models <- fish_long %>% 
  group_by(species) %>% 
  group_split()

# Fit Holt-Winters models and make predictions
predictions <- lapply(species_models, function(species_data) {
  # Extract species name
  species_name <- unique(species_data$species)

  # Convert to time series object
  ts_data <- ts(species_data$counts, frequency = 365, start = c(2001, 1))

  # Fit Holt-Winters model
  hw_model <- HoltWinters(ts_data)

  # Generate forecasts for the next 30 days
  forecasted <- predict(hw_model, n.ahead=30)

  # Return results as a list
  data.frame(
    species = species_name,
    date = seq.Date(from = max(species_data$date) + 1, by = "day", length.out = 30),
    forecast = as.numeric(forecasted)
  )
})

# Combine predictions into a single data frame
forecast_results <- do.call(rbind, predictions)

# View the forecast results
print(forecast_results)


ggplot(forecast_results, aes(x = date, y = forecast, color = species)) +
  geom_line() +
  labs(title = "Holt-Winters Forecasts for Fish Counts",
       x = "Date",
       y = "Forecasted Counts") +
  theme_minimal()
